...
class BucketRunner(object):
    ...
    def __init__(self, bucket_a_size, bucket_b_size, goal):
    ...
    def execute_optimal(self):
        ...
        print("Buckets are sized {} and {}. Goal is {}".format(self.big_bucket.max_volume, self.small_bucket.max_volume, self.goal))

        while self.big_bucket.current_volume != self.goal and self.small_bucket.current_volume != self.goal:
            if self.iterations > 10000:
                raise Exception("yikes! I think something went wrong")
            if self.big_bucket.is_empty():
                print("\tBig bucket is empty. Filling to {} units".format(self.big_bucket.max_volume))
            ...
            self.iterations += 1
            print("Big bucket: {}, small bucket: {}".format(self.big_bucket.current_volume, self.small_bucket.current_volume))
